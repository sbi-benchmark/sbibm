...
def fptd_large(t, w, k):
    ...
    ks = np.arange(1, k+1)
    return (ks * np.exp( - ((ks**2) * (np.pi ** 2) * t) / 2) * np.sin(ks * np.pi * w)).sum() * np.pi
...
def fptd_small(t, w, k):
    ...
    return ((w + (2 * ks)) * np.exp(- ((w + (2 * ks))**2) / (2 * t))).sum() * (1 / np.sqrt(2 * np.pi * (t**3)))
...
def choice_function(t, eps):
    ...
    k_l = int(np.ceil(max(np.sqrt( - (2 * np.log(np.pi * t * eps_l)) / (np.pi**2 * t)), 1 / (np.pi * np.sqrt(t)))))
    k_s = int(np.ceil(max(2 + np.sqrt( - 2 * t * np.log(2 * eps_s * np.sqrt(2 * np.pi * t))), 1 + np.sqrt(t))))
...
def logfptd_batch_python(v, a, w, tau, rt, c, eps=1e-10):
    ...
    for i_b in range(batch_size):
        ...
        logprob[i_b] = np.log(np.array([fptd(t, v[i_b], a[i_b], w[i_b], tau[i_b], eps) for t in signed_rt])).sum()
...
def ddm_batch_python(v, a, w, tau, dt, t_max, num_trials, seed):
    ...
    for i_p in range(num_params):
        for i_t in range(num_trials):
            # loop simulator
            rt[i_p, i_t], c[i_p, i_t] = ddm(v[i_p], a[i_p], w[i_p], tau[i_p], dt=dt, t_max=t_max) 
